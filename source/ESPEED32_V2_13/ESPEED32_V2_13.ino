...
void Task1code(void *pvParameters) 
{
  for (;;) 
  {
    ...
    static bool buttPressToggle = true;
    ...
    switch (g_currState) {
      ...
      case RUNNING: /* when the global variable State is in RUNNING the Task2 will elaborate the trigger to produce the PWM out */
        ...
        /* If the small button is pressed, move the menu to BRAKE or SENSI directly in value selection mode, in order to quick change duiring race  */
        if (digitalRead(PUSHBUTTON_PIN) == BUTTON_PRESSED) 
        {
          menuState = VALUE_SELECTION; // set the menu in to "update values" mode
          if (buttPressToggle)
          {
            keySound();
            g_encoderMainSelector = 1;   // update menu 1 item(BRAKE level )
            menuState = rotary_onButtonClick(ITEM_SELECTION);  /* pretend  the rotary encoder button is pressed in order to go in to VALUE_SELECTION */
          }else
          {
            keySound();
            delay(70);
            keySound();
            g_encoderMainSelector = 2;   // update menu 2 item(SENSI level )
            menuState = rotary_onButtonClick(ITEM_SELECTION);  /* This function is called if the encoder is pressed  */
          }
          g_lastEncoderInteraction = millis(); /* Update last encoder "prretended" interaction */
          buttPressToggle = !buttPressToggle;  /* change small button next behaviour by toggling buttPressToggle */
          while(digitalRead(PUSHBUTTON_PIN) == BUTTON_PRESSED); // remain here until is pressed
        }
        ...
        {
          g_encoderSecondarySelector = g_escVar.encoderPos;         /* If in VALUE_SELECTION, update the encoder SecondaryEncoder with the encoder position */
