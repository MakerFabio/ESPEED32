...
uint16_t throttleAntiSpin3(uint16_t requestedSpeed) 
{
  ...
  else 
  {
    ...
    {
      /* calculate max delta speed  deltaSpeed = ((minSPeed-MaxSpeed)* DeltaTime) / antiSpin (from min speed to max speed) */
      maxDeltaSpeedx1000 = ((g_storedVar.carParam[g_carSel].maxSpeed - g_storedVar.carParam[g_carSel].minSpeed) * (deltaTime_uS)) / (g_storedVar.carParam[g_carSel].antiSpin);  
...
/**
 * Apply antispin calculation (according to antispin settings) to a requested output speed.
 * antispin which is called every 0,5ms, input parameter is requestedSpeed, which ranges from MinSPeed to MaxSPeed. 
 * the output should be a value which is following the input (from MinSpeed to MaxSpeed), but with a maximum variation/time 
 * which is a non linear ramp. the time from minSpeed to MaxSpeed at the output should be a parameter called Antsipin with 
 * measurement unit in ms. So that if the input is making a step from min to max, then output si just following it with a non 
 * linear curve and is taking "Antsipin" time to reach the maximum. Of course, the ramp time is proportional to the output swing amount. 
 * if the requested speed is only MaxSPeed/2 then also the ramp time is Antispin/2
 * @param requestedSpeed [%] The requested outputSpeed at the end of the throttle -> speed pipeline
 * @return [%] The output speed closer to the requestedSpeed that respect the Antispin settings.
 */
uint16_t throttleAntiSpin2(uint16_t requestedSpeed) 
{
  static uint32_t lastOutputSpeedx1000 = 0,maxDeltaLinearSpeedx1000, maxGammaDeltaSpeedx1000, outputSpeedX1000;   /* To keep track of output speeds, in a larger scale to increase granularity */
  static unsigned long prevCall_uS = 0, deltaTime_uS, currCall_uS;    /* To keep track of time between executions */
  uint32_t outputSpeed;     
  ...
  else 
  {
    ...
    {
      // calculate max delta speed in linear dimension
      maxDeltaLinearSpeedx1000 = ((g_storedVar.carParam[g_carSel].maxSpeed - g_storedVar.carParam[g_carSel].minSpeed) * (deltaTime_uS)) / (g_storedVar.carParam[g_carSel].antiSpin);  /* deltaSpeed = ((minSPeed-MaxSpeed)* DeltaTime) / antiSpin (from min speed to max speed) */
  
    
      if (lastOutputSpeedx1000 < ((uint32_t)requestedSpeed * 1000 - maxDeltaLinearSpeedx1000))  /* Check there is room to increase speed by a maxdeltaspeed */
      {
        outputSpeedX1000 = (lastOutputSpeedx1000 + maxDeltaLinearSpeedx1000);
      } 
      else 
      {
        outputSpeedX1000 = requestedSpeed * 1000;
...
uint16_t throttleAntiSpin(uint16_t requestedSpeed) 
{
  ...
  else 
  {
    ...
    else /* Requested speed is increasing (Car is accelerating) here apply antispin */
    {
      /* calculate max delta speed  deltaSpeed = ((minSPeed-MaxSpeed)* DeltaTime) / antiSpin (from min speed to max speed) */
      maxDeltaSpeedx1000 = ((g_storedVar.carParam[g_carSel].maxSpeed - g_storedVar.carParam[g_carSel].minSpeed) * (deltaTime_uS)) / (g_storedVar.carParam[g_carSel].antiSpin);  
      // if (lastOutputSpeedx1000 <((g_storedVar.carParam[g_carSel].maxSpeed + g_storedVar.carParam[g_carSel].minSpeed)*1000)/2 )
      //   maxDeltaSpeedx1000 = 4* maxDeltaSpeedx1000;
      // else
      //   maxDeltaSpeedx1000 = maxDeltaSpeedx1000/2;
